...
class NproductsController extends BaseController {
	...
	function getAdd( $id = null)
	{
		...
		$this->data['tags'] = Tag::getListtags();
		$this->data['tags_avai'] =(array) Tag::getListtagsAvai($id);
		...
		if ($validator->passes()) {
			...
			if(!is_null(Input::file('file')))
			{
				...
				if( $uploadSuccess ) {
				    ...
				    if(Input::get('action') != "")
				    {
				    	...
				    	@unlink(ROOT .'/uploads/products/thumb/three_'.$data_old->image);
				    }
				}
			}
			...
			if(!is_null(Input::file('file2')))
			{
				...
				if( $uploadSuccess ) {
				    ...
				    SiteHelpers::cropImage('263' , '197' , $orgFile ,  $extension,	 $thumbFile);
				    if(Input::get('action') != "")
				    {
				    	$data_old = $this->model->getRow(Input::get('action'));
			...
			$arr_tag = $data['tags_id'] == '' ? array() : $data['tags_id'];
			unset($data['tags_id']);
			$data['SupplierID'] = "1";
			$data['created'] = time();
			...
			DB::table('product_tag')->where('ProductID','=',$ID)->delete();
			if(count($arr_tag) > 0){
				foreach($arr_tag as $tag){
					DB::table('product_tag')->insert(array('ProductID'=>$ID,'tags_id'=>$tag));
				}
			}
